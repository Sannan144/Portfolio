...
const Buttons = ({ onNavigate,btn,prTrue }) => {
  ...
  return (
    <div ref={btn} className='btn-parent fixed z-[30] bottom-2 left-1/2 -translate-x-1/2' style={{ whiteSpace: 'nowrap' }}>
      {btnArr.map((name, index) => {
        return (
          <button
            key={index}
            onClick={() => onNavigate(`/${name}`)}
            style={{ fontFamily: 'roboto',color:prTrue && 'white' }}
